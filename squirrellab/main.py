...
def main():
    global FPSCLOCK, DISPLAYSURF, BASICFONT, L_SQUIR_IMG, R_SQUIR_IMG, GRASSIMAGES, L_SHEALD_IMG, R_SHEALD_IMG, L_POWERUP_SQUIR_IMG, R_POWERUP_SQUIR_IMG
    ...
    L_SHEALD_IMG = pygame.image.load('sheald.png')
    L_SHEALD_IMG = pygame.transform.scale(L_SHEALD_IMG, (25, 30))
    R_SHEALD_IMG = pygame.transform.flip(L_SHEALD_IMG, True, False)

    L_POWERUP_SQUIR_IMG = pygame.image.load("powerUpSquirrel.png")
    R_POWERUP_SQUIR_IMG = pygame.transform.flip(L_POWERUP_SQUIR_IMG, True, False)
    GRASSIMAGES = []
    for i in range(1, 5):
...
def runGame():
    ...
    if POWERUP:
        playerObj = {'surface': pygame.transform.scale(L_POWERUP_SQUIR_IMG, (STARTSIZE, STARTSIZE)),
                     'facing': LEFT,
                     'size': STARTSIZE,
                     'x': HALF_WINWIDTH,
                     'y': HALF_WINHEIGHT,
    ...
    for i in range(1):
        shealdObj.append(makeNewSheald(camerax, cameray))
        shealdObj[i]['x'] = random.randint(0, WINWIDTH)
        shealdObj[i]['y'] = random.randint(0, WINHEIGHT)
    ...
    while True:  # main game loop
        ...
        for sObj in squirrelObjs:
            ...
            bounce_amount = getBounceAmount(sObj['bounce'], sObj['bouncerate'], sObj['bounceheight'])
            if sObj['bounce_direction'] == 'up':
                sObj['y'] -= bounce_amount
            elif sObj['bounce_direction'] == 'down':
                sObj['y'] += bounce_amount
        ...
        for i in range(len(squirrelObjs) - 1, -1, -1):
            if isOutsideActiveArea(camerax, cameray, squirrelObjs[i]):
                del squirrelObjs[i]
        for i in range(len(shealdObj)-1, -1, -1):
            if isOutsideActiveArea(camerax, cameray, shealdObj[i]):
                del shealdObj[i]
        ...
        while len(shealdObj) < NUMBERSHEALDS:
            shealdObj.append(makeNewSheald(camerax, cameray))
        ...
        for shObj in shealdObj:
            shObj['rect'] = pygame.Rect((shObj['x'] - camerax,
                                        shObj['y'] - cameray,
                                        shObj['width'],
                                        shObj['height']))
            DISPLAYSURF.blit(shObj['shealdImage'], shObj['rect'])
            ...
            if event.type == QUIT:
                ...
                if event.key in (K_UP, K_w):
                    ...
                    if playerObj['facing'] != LEFT:  # change player image
                        if POWERUP:
                            playerObj['surface'] = pygame.transform.scale(L_POWERUP_SQUIR_IMG,
                    ...
                    if playerObj['facing'] != RIGHT:  # change player image
                        if POWERUP:
                            playerObj['surface'] = pygame.transform.scale(R_POWERUP_SQUIR_IMG,
                                                                          (playerObj['size'], playerObj['size']))
                        else:
        ...
        if not gameOverMode:
            ...
            for i in range(len(squirrelObjs) - 1, -1, -1):
                ...
                if 'rect' in sqObj and playerObj['rect'].colliderect(sqObj['rect']):
                    ...
                    if sqObj['width'] * sqObj['height'] <= playerObj['size'] ** 2 or POWERUP:
                        ...
                        if POWERUP and shealdDurability > 0:
                            shealdDurability -= 1
                        elif POWERUP:
                            setPowerUp(False)
                            shealdDurability = 3
                    elif sqObj['width'] * sqObj['height'] > playerObj['size'] ** 2 and playerObj['size'] >= LOSESIZE:
                        playerObj['size'] -= int((sqObj['width'] * sqObj['height']) ** 0.02) + 1
            ...
            for i in range(len(shealdObj)):
                if playerObj['rect'].colliderect(shealdObj[i]['rect']):
                    del shealdObj[i]
                    setPowerUp(True)
...
def makeNewSquirrel(camerax, cameray):
    ...
    sq['bounce_direction'] = random.choice(
        ['up', 'down'])  # Vo dictionary dodadov i bounce_direction koje ili up ili down
...
def makeNewSheald(camerax, cameray):
    sh = {}
    sh['shealdImage'] = L_SHEALD_IMG
    sh['width'] = L_SHEALD_IMG.get_width()
    sh['height'] = L_SHEALD_IMG.get_height()
    sh['x'], sh['y'] = getRandomOffCameraPos(camerax, cameray, sh['width'], sh['height'])
    sh['rect'] = pygame.Rect((sh['x'], sh['y'], sh['width'], sh['height']))
    return sh
