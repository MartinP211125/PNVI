...
def runGame():
    ...
    while True:  # main game loop
        ...
        prob = random.randint(1, 5*FPS)
        if prob == 1 and blueApple is None:
        ...
        elif blueApple is not None and wormCoords[HEAD]['x'] == blueApple['x'] and wormCoords[HEAD]['y'] == blueApple['y']:
            setFPS(FPS - 5)
            blueApple = None
            prob = 0
        else:
...
def drawBlueApple(coord):
    if coord is not None:
        ...
        y = coord['y'] * CELLSIZE
        appleRect = pygame.Rect(x, y, CELLSIZE, CELLSIZE)
        pygame.draw.rect(DISPLAYSURF, (0, 0, 255), appleRect)
